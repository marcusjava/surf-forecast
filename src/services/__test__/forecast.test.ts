...
describe('Forecast Service', () => {
  ...
  it('should return the forecast for mutiple beaches in the same hour with different ratings ordering by rating', async () => {
  ...
  it('should return a forecast for a list of beaches', async () => {
    /*  StormGlass.prototype.fetchPoints = jest
      .fn()
      .mockResolvedValue(stormGlass3HourNormalized); */

    mockedStormGlassService.fetchPoints.mockResolvedValue(
      stormGlass3HourNormalized
    );
  ...
  it('should returna an empty list when the beaches array is empty', async () => {
    const forecast = new Forecast();
    const beaches = await forecast.processForecastForBeaches([]);

    expect(beaches).toEqual([]);
  });

  it('should throw internal processimg error when something goes wrong during the rating process', async () => {
    const beaches: Beach[] = [
      {
        lat: -33.792726,
        lng: 151.289824,
